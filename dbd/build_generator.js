...
if (mode == 'Killer') {
    ...
    alert('Please click on the DBD Logo and navigate the site using the buttons.');
}
...
if (build != 'Semi-Random') {
    ...
    if (style_1 == style_2) {
        $('#build-description').text('This build will help you ' + style_1.description + '.');
    } else {
        $('#build-description').text('This build will help you ' + style_1.description + ' and ' + style_2.description + '.');
    }
}
...
function animateRandom() {
    ...
    for (let refreshes = 0; refreshes < 60; refreshes++) {
        (function(refreshes) {
            setTimeout(() => {
                for (let i = 0; i < 4; i++) {
                    let perk_name = Object.keys(perk_set)[Math.floor(Math.random() * Object.keys(perk_set).length)];
                    $('#perk' + (i + 1).toString() + '-img').attr('src', perk_set[perk_name].img_url);
                }
            }, 25 + 25 * refreshes);
        })(refreshes);
    }
...
function fetchAndFade(perks) {
    ...
    for (let perk in perks) {
        ...
        setTimeout((dom_img, dom_caption) => {
            dom_img.removeClass('fadeIn');
            dom_caption.removeClass('fadeIn');
        }, 1500, $('#perk' + (i + 1).toString() + '-img'), $('#perk' + (i + 1).toString() + '-caption'));
        i++;
    }
}
...
function styledBuild() {
    ...
    let perk_name_1 = style_1.perks[Math.floor(Math.random() * style_1.perks.length)];
    perks[perk_name_1] = perk_set[perk_name_1];

    let uniquePerk = true;
    do {
        uniquePerk = true;
        let perk_name_2 = style_2.perks[Math.floor(Math.random() * style_2.perks.length)];
    ...
    for (let i = 2; i < 4; i++) {
        ...
        do {
            ...
            let perk_name;
            if (style == 1) {
                perk_name = style_1.perks[Math.floor(Math.random() * style_1.perks.length)]
            } else {
                perk_name = style_2.perks[Math.floor(Math.random() * style_2.perks.length)]
            }
...
function presetBuild() {
    ...
    for (let refreshes = 0; refreshes < 60; refreshes++) {
        (function(refreshes) {
            setTimeout(() => {
                for (let i = 0; i < 4; i++) {
                    let tmp_main = Object.keys(main_source)[Math.floor(Math.random() * Object.keys(main_source).length)];
                    $('#character-img').attr('src', main_source[tmp_main].img_url);
                    let tmp_addon = Object.keys(addons)[Math.floor(Math.random() * Object.keys(addons).length)];
                    $('#addon1-img').attr('src', addons[tmp_addon].img_url);
                    tmp_addon = Object.keys(addons)[Math.floor(Math.random() * Object.keys(addons).length)];
    ...
    setTimeout(() => {
        ...
        setTimeout((dom_img, dom_caption) => {
            dom_img.removeClass('fadeIn');
            dom_caption.removeClass('fadeIn');
        }, 1500, $('#character-img'), $('#character-caption'));
        ...
        setTimeout((dom_img, dom_caption) => {
            dom_img.removeClass('fadeIn');
            dom_caption.removeClass('fadeIn');
        }, 1500, $('#addon1-img'), $('#addon1-caption'));
        ...
        setTimeout((dom_img, dom_caption) => {
            dom_img.removeClass('fadeIn');
            dom_caption.removeClass('fadeIn');
        }, 1500, $('#addon2-img'), $('#addon2-caption'));
        $('#title').empty().append('<strong>' + build_name + '</strong>');
        $('#explanation').empty().append('<strong>' + chosen_build.meme + '</strong>: ' + chosen_build.explanation);
        $('#title').addClass('fadeIn');
        $('#explanation').addClass('fadeIn');
        setTimeout((title, explanation) => {
            title.removeClass('fadeIn');
            explanation.removeClass('fadeIn');
        }, 1500, $('#title'), $('#explanation'));
        let i = 0;
        for (let perk in perks) {
            // display unique perk in appropriate section of the page
            $('#perk' + (i + 1).toString() + '-url').attr('href', perks[perk].url);
...
function displayBuild() {
    switch (build) {
        case 'Random':
            animateRandom();
            setTimeout(() => { fetchAndFade(randomBuild()) }, 1501);
            break;
        case 'Semi-Random':
            animateRandom();
            setTimeout(() => { fetchAndFade(styledBuild()) }, 1501);
            break;
        case 'Preset':
            presetBuild();
            break;
